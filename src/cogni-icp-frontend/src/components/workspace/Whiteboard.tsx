...
const Whiteboard: React.FC<WhiteboardProps> = ({ workspaceId, onClose }) => {
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on('workspace_update', (update: { elements: DrawingElement[] }) => {
  ...
  const drawElements = (elementsToRender: DrawingElement[]) => {

...
import { CreatePollParams } from '../../services/studyGroupService';
import { IoAdd, IoTrash, IoCalendar, IoHelpCircle, IoCheckmark } from 'react-icons/io5';

interface PollCreatorProps {
  onSubmit: (data: CreatePollParams) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
}
...
const PollCreator: React.FC<PollCreatorProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validOptions = options.filter(option => option.trim() !== '');
      const pollData: CreatePollParams = {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            min={new Date().toISOString().slice(0, 16)} // Current datetime in format required by datetime-local

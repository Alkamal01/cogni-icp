...
import { chatService, Message, ChatEvent } from '../../services/chatService';
...
const DiscussionModal: React.FC<DiscussionModalProps> = ({
  ...
  const initializeChat = useCallback(async (groupId: string) => {
    if (!groupId) return;
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 flex items-center justify-center p-4">
        ...
        >
          ...
          >
            ...
            {messages.length === 0 && !loading ? (
              ...
              <div
                key={message.id}
                className={`flex ${
                    message.user.id === 0 // Assuming 0 means the current user (or we can get the current user's ID)
                    ? 'justify-end'
                    : 'justify-start'
                }`}
              >
                <div
                  className={`flex max-w-[80%] ${
                      message.user.id === 0 ? 'flex-row-reverse' : 'flex-row'
                  }`}
                >
                    {message.user.id !== 0 && (
                    <div className="flex-shrink-0 mr-3">
                        ...
                        <div className="h-10 w-10 rounded-full bg-gradient-to-r from-primary-500 to-purple-500 flex items-center justify-center text-white font-medium">
                  ...
                  <div
                    className={`flex flex-col ${
                        message.user.id === 0 ? 'items-end mr-3' : 'items-start'
                    }`}
                  >
                    <div
                        ...
                        <div className="mt-2 pt-2 border-t border-gray-200/20">
                          {message.attachments.map((attachment, idx) => (
                            <a
                              ...
                              className="flex items-center text-xs text-blue-200 hover:text-blue-100"
                            >
          ...
          {typingUsers.length > 0 && (
            <div className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-800">

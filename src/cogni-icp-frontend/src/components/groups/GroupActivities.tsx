...
import { Poll, StudySession, CreatePollParams, StudySessionParams } from '../../services/studyGroupService';
...
interface GroupActivitiesProps {
  ...
  onCreatePoll: (data: CreatePollParams) => Promise<void>;
  ...
  onCreateSession: (data: StudySessionParams) => Promise<void>;
  onJoinSession: (sessionId: number) => Promise<void>;
  onLeaveSession: (sessionId: number) => Promise<void>;
...
const GroupActivities: React.FC<GroupActivitiesProps> = ({
  groupId,
  polls,
  sessions,
  onCreatePoll,
  ...
  const [activeTab, setActiveTab] = useState<'polls' | 'sessions'>('polls');
  ...
  const canManageItem = (creatorUserId: number) => {
              ...
              {sortedPolls.map(poll => {
                ...
                return (
                  <PollDisplay
                    ...
                    poll={poll}
                    onVote={onVotePoll}
                    onClose={canManage ? onClosePoll : undefined}
                    onDelete={canManage ? onDeletePoll : undefined}
                    isAdmin={isAdmin}
                    isCreator={userId === poll.creator_id}
                  />
                );
              })}
            </div>
              ...
              {upcomingSessions.map(session => {
                ...
                return (
                  <SessionDisplay
                    ...
                    onDelete={canManage ? onDeleteSession : undefined}
                    isAdmin={isAdmin}

...
const Chat = () => {
  ...
  const addChat = async () => {
    if (chatName.trim() === "" || chatObjective.trim() === "") return;

    try {
      const token = Cookies.get("access_token");
      const response = await axios.post(
        "api/topics",
        { title: chatName, objectives: chatObjective },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "api/interact",
        { topic_id: selectedChat.id, message: userMessage },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const saveEditedMessage = () => {
    const updatedChats = chats.map((chat) =>
      chat.id === editingMessage.chatId
        ? {
            ...chat,
            messages: chat.messages.map((msg, index) =>
              index === editingMessage.messageIndex
                ? { ...msg, userMessage }
                : msg
            )
          }
  ...
  return (
    <div className="flex h-full md:p-6 md:space-x-4 relative"> {/* Added relative, adjusted padding for mobile */}
      {/* Overlay for mobile when chat list is open */}
      {showChatList && !window.matchMedia("(min-width: 768px)").matches && (
        <div 
      ...
      {showChatList && ( // Restored conditional rendering
        ...
        <button
            className="bg-[#000053] text-white px-4 py-2 rounded mb-4 flex items-center space-x-2 w-full justify-center"
            onClick={openModal}
          >
            <FaPlus />
              ...
              chats.map((chat) => (
                <li
                  ...
                  className={`p-2 rounded-lg cursor-pointer ${
                    selectedChat?.id === chat.id ? "bg-blue-200" : "bg-gray-200"
                  }`}
                >
                  ...
                  <div className="mt-2">
                    <div className="bg-gray-300 rounded-full h-2">
                      <div
                        className="bg-[#000053] h-2 rounded-full"
                        style={{                          width: `${progress[chat.id] || 0}%` 
                      }}
                      />
                    </div>
                  </div>
...
<div className="flex-1 bg-white p-4 md:p-6 rounded-lg shadow-lg md:ml-0 w-full"> 
  {selectedChat ? (
    <div className="flex flex-col h-full">
      ...
      <div className="flex justify-between items-center mb-4 md:mb-6 pb-4 border-b">
        <div className="flex items-center"> {/* Wrapper for toggler and title/objective */}
          <button 
            className="md:hidden text-gray-700 mr-3 p-2 -ml-2"
              ...
              <div className="bg-blue-500 text-white rounded-lg px-3 py-2 md:px-4 md:py-3 max-w-[85%] sm:max-w-[80%]"> {/* Adjusted padding and max-width */}
                <div className="flex justify-between items-start mb-1">
                  ...
                  <div className="flex space-x-2 ml-3 md:ml-4">
                    <button 
                      onClick={() => handleEditMessage(selectedChat.id, index)}
      ...
      <div className="border-t pt-3 md:pt-4">
        <div className="flex items-center space-x-2 md:space-x-3">
          <div className="flex-1 relative">
            <textarea
              className="w-full border rounded-lg px-3 py-2 md:px-4 md:py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none text-sm md:text-base"
              value={userMessage}
          ...
          <button
            className="bg-[#000053] hover:bg-[#000075] text-white px-4 py-3 md:px-6 rounded-lg flex items-center transition-colors duration-200" // Adjusted padding
    ...
    <div className="flex flex-col items-center justify-center h-full text-center p-4 md:p-8">
      ...
      <button 
        className="md:hidden text-gray-700 absolute top-5 left-5 p-2" 
        onClick={() => setShowChatList(true)}
      >
    ...
    {/* Chat Creation Modal */}
        {isModalOpen && (
          <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4"> {/* Ensure modal is on top and has padding */}
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md"> {/* Increased padding and max-width */}
              ...
              <input
                type="text"
                className="border rounded p-3 w-full mb-3 text-sm md:text-base" /* Adjusted padding and margin */
              ...
              <div className="flex justify-between">
                <button
                  ...
                  onClick={selectedChat ? saveEdit : addChat}
                >

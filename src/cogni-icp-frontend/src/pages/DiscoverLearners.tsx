...
import connectionService, { SuggestedConnection } from '../services/connectionService';
...
interface LearnerProfile {
  ...
  lastActive: string;
  location?: string;
  studyPreference: 'individual' | 'group' | 'both';
...
const DiscoverLearners: React.FC = () => {
  const { user } = useAuth();
  const { showToast } = useToast();
  
  // State management
  const [learners, setLearners] = useState<LearnerProfile[]>([]);
  const [recommendations, setRecommendations] = useState<LearnerProfile[]>([]);
  ...
  const handleSendConnectionRequest = async (learnerId: string) => {
    ...
    try {
      ...
      setRecommendations(prev => prev.map(learner => 
        learner.id === learnerId 
          ? { ...learner, connectionStatus: 'pending' }
          : learner
      ));
  ...
  const getCurrentLearners = () => {
    switch (activeTab) {
      case 'recommended':
        return recommendations.filter(learner => 
  ...
  const LearnerCard: React.FC<{ learner: LearnerProfile }> = ({ learner }) => (
    <motion.div
      layout
    ...
    >
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-2">
              {learner.compatibilityScore && (
                <div className="flex items-center bg-primary-100 dark:bg-primary-900/30 px-2 py-1 rounded-full">
          ...
          {learner.isRecommended && learner.recommendationReason && (
            <div className="mb-3">
              ...
              <ul className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
                {learner.recommendationReason.slice(0, 2).map((reason, index) => (
          ...
          <div className="flex items-center space-x-2 mt-4">
            ...
            {learner.connectionStatus === 'connected' && (
              <Button
                size="sm"
                variant="outline"
                className="flex-1"
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          <AnimatePresence>
            {getCurrentLearners().map((learner) => (
      ...
      {getCurrentLearners().length === 0 && !isLoading && (
            ...
            >
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  placeholder="Hi! I'd love to connect and maybe study together..."
                  rows={4}
                  className="w-full p-3 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                />
              </div>
              ...
              <div className="flex space-x-3">
                ...
                <Button
                  onClick={() => handleSendConnectionRequest(selectedLearner.id)}
                  disabled={sendingRequest === selectedLearner.id}
                  className="flex-1"
                >

...
const LearningPairs = ({ isOpen, onClose, groupId, groupName }) => {
    // Mock data for potential learning pairs
    const [potentialPairs, setPotentialPairs] = useState([
        {
            id: 'user1',
            name: 'Emily Chen',
    ...
    const filteredPairs = potentialPairs.filter(pair => {
        ...
        if (searchTerm) {
            ...
            return (pair.name.toLowerCase().includes(searchLower) ||
    ...
    const handleSendRequest = (pairId) => {
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-850">
            <div className="flex flex-wrap gap-3 items-center">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white" placeholder="Search by name, skills, or interests..."/>
              </div>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              {availableFilters.map((filter) => (<button key={filter} onClick={() => toggleFilter(filter)} className={`px-3 py-1 text-sm rounded-full transition-colors ${selectedFilters.includes(filter)
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {filteredPairs.map((pair) => (<div key={pair.id} className="border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden bg-white dark:bg-gray-900 shadow-sm hover:shadow-md transition-shadow">
                  <div className="p-5">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        {pair.avatar ? (<img src={pair.avatar} alt={pair.name} className="h-12 w-12 rounded-full object-cover"/>) : (<div className="h-12 w-12 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 dark:from-primary-600 dark:to-primary-800 flex items-center justify-center text-white text-lg font-semibold">
                            {pair.name.charAt(0)}
                          </div>)}
                        <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {pair.skills.map((skill, idx) => (<span key={idx} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
                              {skill}
                            </span>))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {pair.interests.map((interest, idx) => (<span key={idx} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300">
                              {interest}
                            </span>))}
                        </div>
                      </div>
                    ...
                    <div className="mt-5 pt-4 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center">
                      ...
                      {connectionMessages[pair.id] === 'success' ? (<div className="flex items-center text-green-600 dark:text-green-400 text-sm font-medium">
                          <Check className="h-4 w-4 mr-1"/>
                          Request Sent
                        </div>) : connectionMessages[pair.id] === 'pending' ? (<div className="flex items-center text-amber-600 dark:text-amber-400 text-sm font-medium">
                          ...
                          <Button variant="primary" size="sm" className="flex items-center" onClick={() => handleSendRequest(pair.id)}>
                            <span>Connect</span>
            ...
            {filteredPairs.length === 0 && (<div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">

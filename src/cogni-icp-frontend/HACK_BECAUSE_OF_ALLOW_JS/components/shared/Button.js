...
const Button = ({ variant = 'primary', size = 'md', isLoading = false, fullWidth = false, icon = null, iconPosition = 'left', animated = false, className = '', children, disabled, ...props }) => {
    // Base button styles
    ...
    return (<button className={`${buttonStyles} ${className}`} disabled={disabled || isLoading} {...props}>
      {isLoading ? (<span className="inline-flex items-center">

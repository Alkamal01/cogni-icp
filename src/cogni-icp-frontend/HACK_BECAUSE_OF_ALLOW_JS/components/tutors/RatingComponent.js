...
const RatingComponent = ({ tutorId, onRatingSubmitted }) => {
    ...
    const averageRating = ratings.length > 0
        ...
        : 0;
    return (<div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center">
          <div className="flex items-center mr-2">
            {[1, 2, 3, 4, 5].map((star) => (<Star key={star} className={`w-4 h-4 ${star <= averageRating
      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="flex items-center">
          {[1, 2, 3, 4, 5].map((star) => (<button key={star} onClick={() => handleSubmitRating(star)} disabled={isSubmitting} className={`p-1 ${star <= (userRating?.rating || 0)
                ? 'text-yellow-400'
                : 'text-gray-300 hover:text-yellow-400'}`}>
              <Star className="w-4 h-4"/>
            </button>))}
        </div>
        {userRating && (<Button variant="outline" size="sm" onClick={handleDeleteRating} disabled={isDeleting} className="ml-2 text-red-600 hover:text-red-700">

...
import { BookOpen, Clock, CheckCircle, ArrowRight, ChevronDown as ArrowLeftIcon, MessageSquare as PlayIcon, BookOpen as FileTextIcon, Zap, Lock, User, Users, MessageSquare, AlertCircle, Brain } from 'lucide-react';
...
const LearningPathDetail = () => {
    ...
    const startOrContinueModule = (moduleId) => {
    ...
    const overallProgress = Math.round((learningPath.modules.reduce((acc, module) => acc + (module.progress || 0), 0)) /
    ...
    return (<div className="space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="bg-gradient-to-r from-primary-500/10 to-purple-500/10 dark:from-primary-900/30 dark:to-purple-900/30 p-8 rounded-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <Button variant="outline" size="sm" icon={<ArrowLeftIcon className="h-4 w-4"/>} onClick={() => navigate('/learning-paths')} className="mb-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <span className="inline-flex items-center text-amber-500 text-sm">
                  {Array(5).fill(0).map((_, i) => (<svg key={i} className={`w-4 h-4 ${i < Math.floor(learningPath.rating) ? 'text-amber-500' : 'text-gray-300 dark:text-gray-600'}`} fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="flex items-center">
              <div className="flex-shrink-0 mr-4">
                <img src={learningPath.instructor.avatar} alt={learningPath.instructor.name} className="h-12 w-12 rounded-full object-cover border-2 border-white dark:border-gray-800"/>
              </div>
              <div>
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
          ...
          {moduleDetail.status === 'in-progress' ? (<Button variant="primary" size="sm" icon={<PlayIcon className="h-4 w-4"/>} onClick={() => startOrContinueModule(moduleDetail.id)} className="mt-2 sm:mt-0">
              Continue Learning
            </Button>) : moduleDetail.status === 'available' ? (<Button variant="gradient" size="sm" icon={<PlayIcon className="h-4 w-4"/>} onClick={() => startOrContinueModule(moduleDetail.id)} className="mt-2 sm:mt-0">
              Start Next Module
            </Button>) : null}
        </div>
        
        <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
          <div className="h-full bg-primary-600 dark:bg-primary-500 rounded-full" style={{ width: `${overallProgress}%` }}/>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          ...
          <div className="space-y-3">
            {learningPath.modules.map((module) => (<Card key={module.id} className={`p-4 cursor-pointer border-l-4 transition-all ${selectedModule === module.id
                ? 'border-l-primary-600 dark:border-l-primary-500'
                : module.status === 'locked'
                    ? 'border-l-gray-300 dark:border-l-gray-700 opacity-70'
                    : 'border-l-transparent hover:border-l-gray-300 dark:hover:border-l-gray-700'}`} onClick={() => module.status !== 'locked' && setSelectedModule(module.id)}>
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h3 className={`font-medium ${module.status === 'locked'
                ...
                {module.status === 'in-progress' && module.progress !== undefined && (<div className="mt-3 w-full h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-primary-600 dark:bg-primary-500" style={{ width: `${module.progress}%` }}/>
                  </div>)}
              </Card>))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
            ...
            {moduleDetail.resources && moduleDetail.resources.length > 0 && (<div className="mb-6">
                <h3 className="font-semibold text-gray-900 dark:text-white mb-3">
                  Resources
                </h3>
                <div className="space-y-2">
                  {moduleDetail.resources.map(resource => (<a key={resource.id} href={resource.url} target="_blank" rel="noopener noreferrer" className="flex items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
              {moduleDetail.status === 'completed' ? (<div className="flex flex-col sm:flex-row justify-between items-center">
                  <div className="flex items-center text-green-600 dark:text-green-400 mb-4 sm:mb-0">
                    <CheckCircle className="h-5 w-5 mr-2"/>
                    <span className="font-medium">Module Completed</span>
                  </div>
                  <Button variant="outline" icon={<ArrowRight className="h-4 w-4"/>} onClick={() => {
                const nextModule = learningPath.modules.find(m => m.status === 'in-progress' ||
                ...
                </div>) : moduleDetail.status === 'in-progress' ? (<Button variant="primary" fullWidth icon={<PlayIcon className="h-4 w-4"/>} onClick={() => startOrContinueModule(moduleDetail.id)}>
                  Continue Learning
                </Button>) : moduleDetail.status === 'available' ? (<Button variant="gradient" fullWidth icon={<PlayIcon className="h-4 w-4"/>} onClick={() => startOrContinueModule(moduleDetail.id)}>
                  Start Module
                </Button>) : (<div className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">

...
const AnimatedConversationTest = () => {
    ...
    return (<div className="relative w-full max-w-md mx-auto h-[400px] rounded-xl overflow-hidden shadow-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
      <h2 className="text-center text-xl font-bold p-4">Animated Conversation Test</h2>
      
      {/* Chat area */}
      <div ref={conversationRef} className="overflow-y-auto px-4 py-3 h-[300px]">
        <AnimatePresence>
          {messages.map((message) => (<motion.div key={message.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} mb-4`}>
              <div className={`flex max-w-[80%] ${message.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`rounded-xl p-3 ${message.sender === 'user'
                ? 'bg-blue-500 text-white'
                : 'bg-gray-100 text-gray-800'}`}>
                  {message.text}
                  {message.typing && (<div className="flex space-x-1 mt-1 h-5">

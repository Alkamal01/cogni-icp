...
const StudyGroupCard = ({ group, onJoin, onViewDetails }) => {
    ...
    return (<div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow duration-200">
      <div className="p-5">
        <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex items-center space-x-1">
            {group.is_private ? (<Lock className="h-4 w-4 text-gray-400"/>) : (<span className="text-xs text-gray-400">Public</span>)}
          </div>
        </div>
        ...
        <div className="flex flex-wrap mb-4 gap-2">
          {group.topic_name && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400">
              {group.topic_name}
            </span>)}
        ...
        <div className="flex flex-col space-y-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          {group.meeting_frequency && (<div className="flex items-center">
              <span className="text-xs mr-2">📅</span>
              <span>{group.meeting_frequency}</span>
            </div>)}
          
          {group.learning_level && (<div className="flex items-center">
      ...
      <div className="flex space-x-2 p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          {!group.is_member && (<Button variant="primary" onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                onJoin();
            }} disabled={group.member_count >= group.max_members} className="flex-1">
              <span className="flex items-center justify-center">
                Join Group
              </span>
            </Button>)}

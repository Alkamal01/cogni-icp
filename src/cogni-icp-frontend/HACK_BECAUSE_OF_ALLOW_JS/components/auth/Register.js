...
const Register = () => {
    ...
    const handleZkLoginCallback = async (jwt) => {
        try {
            ...
            const email = result.decodedJwt?.email || formData.email || `${result.zkLoginAddress.slice(0, 8)}@zklogin.user`;
      ...
      <div className="w-full max-w-md">
        ...
        <div className="shadow-xl border-0 rounded-xl bg-white dark:bg-gray-800 relative">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input id="first_name" value={formData.first_name} onChange={handleInputChange} onBlur={handleBlur} placeholder="Enter your first name" required className={`w-full px-4 py-3 rounded-md focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${showError('first_name') ? 'border border-red-500 focus:ring-red-400' : 'border border-gray-300 dark:border-gray-600 focus:ring-blue-400'}`}/>
                  {showError('first_name') && (<p className="mt-1 text-sm text-red-500">{errors.first_name}</p>)}
                </div>
                <div className="space-y-2">
                  <label htmlFor="last_name" className="flex items-center space-x-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                    <span>Last Name</span>
                  </label>
                  <input id="last_name" value={formData.last_name} onChange={handleInputChange} onBlur={handleBlur} placeholder="Enter your last name" required className={`w-full px-4 py-3 rounded-md focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${showError('last_name') ? 'border border-red-500 focus:ring-red-400' : 'border border-gray-300 dark:border-gray-600 focus:ring-blue-400'}`}/>
                  {showError('last_name') && (<p className="mt-1 text-sm text-red-500">{errors.last_name}</p>)}
                </div>
              </div>
              <div className="space-y-2">
                ...
                <input id="username" value={formData.username} onChange={handleInputChange} onBlur={handleBlur} placeholder="Choose a username" required className={`w-full px-4 py-3 rounded-md focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${showError('username') ? 'border border-red-500 focus:ring-red-400' : 'border border-gray-300 dark:border-gray-600 focus:ring-blue-400'}`}/>
                {showError('username') && (<p className="mt-1 text-sm text-red-500">{errors.username}</p>)}
              </div>
              <div className="space-y-2">
                <label htmlFor="email" className="flex items-center space-x-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                  <span>Email</span>
                </label>
                <input id="email" type="email" value={formData.email} onChange={handleInputChange} onBlur={handleBlur} placeholder="Enter your email" required className={`w-full px-4 py-3 rounded-md focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${showError('email') || emailAlreadyRegistered ? 'border border-red-500 focus:ring-red-400' : 'border border-gray-300 dark:border-gray-600 focus:ring-blue-400'}`}/>
                {showError('email') && (<p className="mt-1 text-sm text-red-500">{errors.email}</p>)}
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input id="password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={handleInputChange} onBlur={handleBlur} placeholder="Create a strong password" required className={`w-full px-4 py-3 rounded-md focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${showError('password') ? 'border border-red-500 focus:ring-red-400' : 'border border-gray-300 dark:border-gray-600 focus:ring-blue-400'}`}/>
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">
                    {showPassword ? <FaEyeSlash /> : <FaEye />}
                  </button>
                </div>
                {showError('password') && (<div className="mt-1 space-y-1">
                    {formData.password.length < 8 && (<p className="text-sm text-red-500">Password must be at least 8 characters.</p>)}
                    {!/[a-z]/.test(formData.password) && (<p className="text-sm text-red-500">Password must include at least one lowercase letter.</p>)}
                    {!/[A-Z]/.test(formData.password) && (<p className="text-sm text-red-500">Password must include at least one uppercase letter.</p>)}
                    {!/[0-9]/.test(formData.password) && (<p className="text-sm text-red-500">Password must include at least one number.</p>)}
                    {!/[!@#$%^&*]/.test(formData.password) && (<p className="text-sm text-red-500">Password must include at least one special character (!@#$%^&*).</p>)}
                  </div>)}
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input id="confirm_password" type={showConfirmPassword ? 'text' : 'password'} value={formData.confirm_password} onChange={handleInputChange} onBlur={handleBlur} placeholder="Confirm your password" required className={`w-full px-4 py-3 rounded-md focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${showError('confirm_password') ? 'border border-red-500 focus:ring-red-400' : 'border border-gray-300 dark:border-gray-600 focus:ring-blue-400'}`}/>
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">
                    {showConfirmPassword ? <FaEyeSlash /> : <FaEye />}
                  </button>
                </div>
                {showError('confirm_password') && (<p className="mt-1 text-sm text-red-500">{errors.confirm_password}</p>)}
              </div>
              <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-blue-600 to-teal-500 hover:from-blue-700 hover:to-teal-600 text-white font-semibold py-3 rounded-md transition-colors">
              ...
              <div className="flex justify-center gap-4 mb-2">
                <button onClick={() => handleSocialSignUp('google')} type="button" className="w-full flex items-center justify-center py-3 px-4 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                  Sign up with ZK
                </button>
                <button onClick={handleSuiLogin} className="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-r from-blue-600 to-teal-500 hover:from-blue-700 hover:to-teal-600 transition-colors shadow" type="button" aria-label="Register with Sui ZK" disabled={isZkLoginLoading}>
                  {isZkLoginLoading ? (<Loader2 className="w-6 h-6 animate-spin text-white"/>) : (<CreditCard className="w-6 h-6 text-white"/>)}
                </button>
              </div>
              {zkLoginStatus && (<div className="mt-2 p-2 text-center text-sm">
              ...
              <button type="button" onClick={checkDebugLog} className="mt-2 w-full text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                Check Debug Log
              </button>
          ...
          <span>Sui ZK Login</span>
            ...
            {errors.general && (<div className="mb-4 p-3 bg-red-50 text-red-600 rounded-lg text-sm mt-4">
                {errors.general}
                {emailAlreadyRegistered && (<div className="mt-1">
                    <button className="underline font-medium text-blue-500 hover:text-blue-700" onClick={handleResendVerification} disabled={resendingVerification}>
...
const PasswordRequirement = ({ text, satisfied }) => (<div className={`flex items-center text-sm ${satisfied ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>

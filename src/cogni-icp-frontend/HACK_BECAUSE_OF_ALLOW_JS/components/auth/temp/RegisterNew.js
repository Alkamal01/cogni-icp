...
const Register = () => {
    ...
    return (<div className="min-h-screen flex flex-col bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      <div className="relative flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="relative z-10 max-w-md w-full space-y-6 bg-white dark:bg-gray-800 py-8 px-4 shadow-lg sm:rounded-lg sm:px-10 border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex justify-center space-x-4">
            <button onClick={() => handleSocialSignUp('google')} className="flex items-center justify-center w-1/2 px-4 py-2 text-sm font-semibold text-gray-100 bg-gray-700 rounded-full hover:bg-gray-600">
              <img src="https://img.icons8.com/color/20/000000/google-logo.png" alt="Google Icon" className="mr-2"/> Continue Google
            </button>
            <button onClick={handleSuiLogin} className="flex items-center justify-center w-1/2 px-4 py-2 text-sm font-semibold text-gray-100 bg-gradient-to-r from-blue-600 to-teal-500 hover:from-blue-700 hover:to-teal-600 rounded-full" disabled={isZkLoginLoading}>
              {isZkLoginLoading ? (<Loader2 className="h-4 w-4 mr-2 animate-spin"/>) : (<CreditCard className="h-4 w-4 mr-2"/>)}
          ...
          {errors.general && (<div className="px-4 py-3 text-sm text-red-500 bg-red-100 dark:bg-red-900/20 rounded-md">
              {errors.general}
            </div>)}
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <div className="mb-1 flex justify-between">
                ...
                {showError('first_name') && (<span className="text-xs text-red-500">{errors.first_name}</span>)}
              </div>
              <input type="text" id="first-name" value={formData.first_name} onChange={handleInputChange} onBlur={handleBlur} className={getFieldClass('first_name')} placeholder="Your first name" required aria-label="First Name"/>
            </div>
            ...
            <div>
              <div className="mb-1 flex justify-between">
                ...
                {showError('last_name') && (<span className="text-xs text-red-500">{errors.last_name}</span>)}
              </div>
              <input type="text" id="last-name" value={formData.last_name} onChange={handleInputChange} onBlur={handleBlur} className={getFieldClass('last_name')} placeholder="Your last name" required aria-label="Last Name"/>
            </div>
            ...
            <div>
              <div className="mb-1 flex justify-between">
                ...
                {showError('username') && (<span className="text-xs text-red-500">{errors.username}</span>)}
              </div>
              <input type="text" id="username" value={formData.username} onChange={handleInputChange} onBlur={handleBlur} className={getFieldClass('username')} placeholder="Choose a username" required aria-label="Username"/>
            </div>
            ...
            <div>
              <div className="mb-1 flex justify-between">
                ...
                {showError('email') && (<span className="text-xs text-red-500">{errors.email}</span>)}
              </div>
              <input type="email" id="email" value={formData.email} onChange={handleInputChange} onBlur={handleBlur} className={getFieldClass('email')} placeholder="you@example.com" required aria-label="Email"/>
            </div>
            ...
            <div>
              <div className="mb-1 flex justify-between">
                ...
                {showError('password') && (<span className="text-xs text-red-500">{errors.password}</span>)}
              </div>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} id="password" value={formData.password} onChange={handleInputChange} onBlur={handleBlur} className={getFieldClass('password')} placeholder="Create a password" required aria-label="Password"/>
                <button type="button" onClick={togglePasswordVisibility} className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400">
                  {showPassword ? <FaEyeSlash /> : <FaEye />}
                </button>
              </div>
              
              {/* Password requirements */}
              {(touchedFields.password || formData.password) && (<div className="mt-2 space-y-1">
                  {passwordRequirements.map((req, idx) => (<div key={idx} className="flex items-center text-xs">
                      {req.met ? (<FaCheck className="text-green-500 mr-1"/>) : (<FaTimes className="text-red-500 mr-1"/>)}
            ...
            <div>
              <div className="mb-1 flex justify-between">
                ...
                {showError('confirm_password') && (<span className="text-xs text-red-500">{errors.confirm_password}</span>)}
              </div>
              <div className="relative">
                <input type={showConfirmPassword ? 'text' : 'password'} id="confirm-password" value={formData.confirm_password} onChange={handleInputChange} onBlur={handleBlur} className={getFieldClass('confirm_password')} placeholder="Confirm your password" required aria-label="Confirm Password"/>
                <button type="button" onClick={toggleConfirmPasswordVisibility} className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400">
                  {showConfirmPassword ? <FaEyeSlash /> : <FaEye />}
                </button>
              </div>
            </div>

            <button type="submit" className={`w-full py-3 text-white rounded-md transition-all ${isFormValid && !loading ? 'bg-blue-500 hover:bg-blue-600' : 'bg-blue-400 cursor-not-allowed'}`} disabled={!isFormValid || loading}>

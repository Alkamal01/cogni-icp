...
import { Shield, CreditCard, Mail, Loader2 } from 'lucide-react';
import zkLoginService from '../../services/zkLoginService';
// Simple CardContent component
const CardContent = ({ children, className = '' }) => (<div className={`p-6 ${className}`}>{children}</div>);
// Simple Label component
const Label = ({ htmlFor, children, className = '' }) => (<label htmlFor={htmlFor} className={`block text-sm font-medium text-gray-700 mb-2 ${className}`}>
    {children}
  </label>);
export const SuiLogin = ({ onSuccess, onError }) => {
    const { login } = useAuth();
        ...
        <Card className="shadow-xl border-0">
          <CardContent className="p-6">
            <div className="space-y-4">
              {status && (<div className={`p-3 rounded-lg text-sm ${status.includes('successful')
              ...
              <div className="space-y-2">
                ...
                <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email for notifications" className="border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200"/>
              </div>
              ...
              <Button onClick={handleSuiLogin} className="w-full bg-gradient-to-r from-blue-600 to-teal-500 hover:from-blue-700 hover:to-teal-600 text-white" disabled={isConnecting}>
                {isConnecting ? (<>

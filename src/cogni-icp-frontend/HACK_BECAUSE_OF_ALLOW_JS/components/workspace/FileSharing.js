...
import { X, Plus, Clock, MessageSquare, Users, BookOpen, ArrowRight } from 'lucide-react';
const FileSharing = ({ sessionId, groupId }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        if (socket && sessionId) {
            ...
            socket.on('folder_created', (folder) => {
    ...
    const displayedFiles = currentFolder
        ...
        : filteredFiles;
    return (<div className="h-full flex flex-col bg-white dark:bg-gray-800 rounded-lg overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-2">
            <Button size="sm" variant="outline" onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} icon={<MessageSquare className="h-4 w-4"/>}>
              {viewMode === 'grid' ? 'List View' : 'Grid View'}
            </Button>
            
            <Button size="sm" onClick={() => setShowNewFolderModal(true)} icon={<Plus className="h-4 w-4"/>}>
              New Folder
            </Button>
            
            <div className="relative">
              <input type="file" id="file-upload" className="sr-only" onChange={handleFileUpload} disabled={isUploading} multiple/>
              <label htmlFor="file-upload" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 cursor-pointer">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Search files..." className="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md leading-5 bg-white dark:bg-gray-800 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
        </div>
      </div>
      
      {/* Upload progress */}
      {isUploading && (<div className="p-4 bg-gray-50 dark:bg-gray-750 border-b border-gray-200 dark:border-gray-700">
      ...
      {currentFolder && (<div className="p-4 bg-gray-50 dark:bg-gray-750 border-b border-gray-200 dark:border-gray-700">
          <nav className="flex" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-2">
              <li>
                <button onClick={() => setCurrentFolder(null)} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
                  All Files
                </button>
              </li>
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        ...
        {!currentFolder && folders.length > 0 && (<div className="mb-6">
            ...
            <div className={viewMode === 'grid' ? 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4' : 'space-y-2'}>
              {folders.map(folder => (<div key={folder.id} onClick={() => setCurrentFolder(folder.id)} className={viewMode === 'grid'
                  ...
                  <div className={viewMode === 'grid' ? 'flex flex-col items-center' : 'flex items-center'}>
                    ...
                    <div className={viewMode === 'grid' ? 'text-center' : ''}>
                      ...
                      {viewMode === 'list' && (<p className="text-xs text-gray-500 dark:text-gray-400">
        ...
        <div>
          ...
          {displayedFiles.length === 0 ? (<div className="text-center py-12">
              ...
              {!searchQuery && (<div className="relative inline-block">
                  <input type="file" id="empty-upload" className="sr-only" onChange={handleFileUpload} disabled={isUploading} multiple/>
                  <label htmlFor="empty-upload" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 cursor-pointer">
                    <Plus className="h-4 w-4 mr-2"/>
                    Upload Files
                  </label>
                </div>)}
            </div>) : (<div className={viewMode === 'grid' ? 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4' : 'space-y-2'}>
              {displayedFiles.map(file => (<div key={file.id} className={viewMode === 'grid'
                  ...
                  {/* Delete button */}
                  <button onClick={() => deleteFile(file.id)} className="absolute top-2 right-2 p-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">
                    <X className="h-4 w-4"/>
                  </button>
                  
                  {viewMode === 'grid' ? (<>
                      <div className="h-32 flex items-center justify-center mb-2">
                        {file.thumbnail ? (<img src={file.thumbnail} alt={file.name} className="h-full object-contain"/>) : (getFileIcon(file.type))}
                      </div>
                      <div>
                      ...
                      <button onClick={() => window.open(file.url, '_blank')} className="ml-4 inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-primary-700 dark:text-primary-300 bg-primary-100 dark:bg-primary-900 hover:bg-primary-200 dark:hover:bg-primary-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                        <ArrowRight className="h-3 w-3 mr-1"/>
                        Download
                      </button>
                    </>)}
      ...
      {/* New folder modal */}
      {showNewFolderModal && (<div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
              ...
              <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-4">
                      <input type="text" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} placeholder="Folder name" className="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"/>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-gray-50 dark:bg-gray-750 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" onClick={createFolder} className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary-600 text-base font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 sm:ml-3 sm:w-auto sm:text-sm">
                  Create
                </button>
                <button type="button" onClick={() => setShowNewFolderModal(false)} className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-700 text-base font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-650 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                  Cancel
                </button>
              </div>
            </div>

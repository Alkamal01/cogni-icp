...
import { Plus, X, CheckCircle, ArrowRight, BookOpen } from 'lucide-react';
import { motion } from 'framer-motion';
const CodeEditor = ({ sessionId, files = [] }) => {
    const { user } = useAuth();
    ...
    // Save current file
    const saveFile = () => {
        setCodeFiles(prev => prev.map(file => file.id === activeFileId
    ...
    const getHighlightClass = (language) => {
    ...
    return (<div className="flex flex-col h-full border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-2 flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <Button size="sm" variant="ghost" onClick={saveFile} disabled={!isEditing} icon={<CheckCircle className="h-4 w-4"/>}>
            Save
          </Button>
          
          <Button size="sm" variant="ghost" onClick={downloadFile} icon={<ArrowRight className="h-4 w-4"/>}>
            Download
          </Button>
          
          <Button size="sm" variant="ghost" onClick={createNewFile} icon={<Plus className="h-4 w-4"/>}>
            New
          </Button>
        </div>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-750 border-b border-gray-200 dark:border-gray-700 overflow-x-auto whitespace-nowrap p-1 px-2 flex">
        {codeFiles.map((file) => (<button key={file.id} onClick={() => handleFileSwitch(file.id)} className={`mr-2 rounded-md px-3 py-1.5 text-xs font-medium inline-flex items-center ${activeFileId === file.id
                ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm'
                : 'text-gray-600 dark:text-gray-300 hover:bg-white hover:dark:bg-gray-700'}`}>
            {file.name}
            {activeFileId === file.id && codeFiles.length > 1 && (<span className="ml-2 p-0.5 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600" onClick={(e) => {
                    e.stopPropagation();
                    deleteFile();
                }}>
      ...
      <div className="flex-1 overflow-auto relative bg-white dark:bg-gray-800">
        <pre className={`h-full m-0 p-4 text-sm font-mono overflow-auto ${getHighlightClass(language)}`}>
          ...
          <textarea value={currentCode} onChange={(e) => handleCodeChange(e.target.value)} onSelect={(e) => {
            const target = e.target;
            handleCursorChange(target.selectionStart, target.selectionStart !== target.selectionEnd
                ? [target.selectionStart, target.selectionEnd]
                : undefined);
        }} className="w-full h-full bg-transparent text-gray-800 dark:text-gray-200 outline-none resize-none font-mono" spellCheck="false"/>
        </pre>
        
        {/* Show cursors from other users - simplified */}
        {Object.entries(activeUsers).map(([userId, data]) => (<motion.div key={userId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute w-0.5 h-5 bg-red-500" style={{

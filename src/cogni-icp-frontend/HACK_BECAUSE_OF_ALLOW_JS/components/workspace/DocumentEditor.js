...
const DocumentEditor = ({ sessionId, documentId }) => {
    const { user } = useAuth();
    ...
    const [content, setContent] = useState('Start typing your collaborative document here...');
    ...
    useEffect(() => {
        if (socket && sessionId && documentId) {
            ...
            socket.on('user_left_document', (userId) => {
                setCollaborators(prev => {
    ...
    return (<div className="flex flex-col h-full border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      {/* Top toolbar */}
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-2 flex items-center justify-between">
        <div className="flex items-center flex-1">
          ...
          <input type="text" value={title} onChange={handleTitleChange} className="text-sm font-medium text-gray-900 dark:text-white bg-transparent border-none outline-none w-full" placeholder="Document Title"/>
        </div>
        
        <div className="flex space-x-2">
          <Button size="sm" variant={showComments ? "primary" : "ghost"} onClick={() => setShowComments(!showComments)} icon={<MessageSquare className="h-4 w-4"/>}>
            Comments {comments.length > 0 && `(${comments.length})`}
          </Button>
          
          <Button size="sm" variant="ghost" onClick={saveDocument} disabled={!isEditing}>
            Save
          </Button>
        </div>
      </div>
      ...
      {/* Collaborators list */}
      {Object.keys(collaborators).length > 0 && (<div className="bg-gray-50 dark:bg-gray-750 border-b border-gray-200 dark:border-gray-700 p-2 flex overflow-x-auto">
          <div className="text-xs text-gray-500 dark:text-gray-400 mr-2 flex items-center">
            <User className="h-3 w-3 mr-1"/> 
            Collaborators:
          </div>
          
          {Object.entries(collaborators).map(([id, data]) => (<div key={id} className="flex items-center mr-3 px-2 py-1 rounded-full text-xs" style={{ backgroundColor: `${data.color}20`, color: data.color }}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 relative" ref={editorRef}>
          <textarea value={content} onChange={handleContentChange} onSelect={handleCursorChange} className="w-full h-full p-4 text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-800 resize-none outline-none font-sans text-sm leading-relaxed" spellCheck="false"/>
          
          {/* Render cursors for other users */}
          {Object.entries(collaborators).map(([id, data]) => (<div key={id} className="absolute pointer-events-none" style={{
                ...getCursorStyle(data.cursor),
                zIndex: 10
            }}>
              <div className="w-0.5 h-5 absolute" style={{ backgroundColor: data.color }}/>
              <div className="absolute top-0 left-0 transform -translate-y-full whitespace-nowrap rounded px-1 text-xs text-white" style={{ backgroundColor: data.color }}>
                {data.name}
              </div>
            </div>))}
          
          {/* Comment indicators */}
          {comments.map(comment => (<div key={comment.id} className="absolute w-5 h-5 bg-yellow-400 rounded-full flex items-center justify-center text-white text-xs cursor-pointer" style={{
                ...getCursorStyle(comment.position),
                transform: 'translate(-50%, -50%)'
            }} title={`${comment.user.name}: ${comment.content}`} onClick={() => setShowComments(true)}>
              !
            </div>))}
        </div>
        
        {/* Comments panel */}
        {showComments && (<div className="w-64 border-l border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-y-auto">
            <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-sm font-medium text-gray-900 dark:text-white">
                Comments
              </h3>
              <button onClick={() => setShowComments(false)} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
                <X className="h-4 w-4"/>
              </button>
            </div>
            
            <div className="p-3 space-y-3">
              {comments.length === 0 ? (<p className="text-sm text-gray-500 dark:text-gray-400">
                  No comments yet. Select text and click "Add Comment" to start a discussion.
                </p>) : (comments.map(comment => (<div key={comment.id} className="bg-gray-50 dark:bg-gray-750 p-3 rounded-lg">
            ...
            <div className="p-3 border-t border-gray-200 dark:border-gray-700">
              <Button size="sm" variant="outline" className="w-full" onClick={addComment} disabled={!selection || selection.start === selection.end} icon={<Plus className="h-3 w-3"/>}>
                Add Comment
              </Button>
            </div>
          </div>)}

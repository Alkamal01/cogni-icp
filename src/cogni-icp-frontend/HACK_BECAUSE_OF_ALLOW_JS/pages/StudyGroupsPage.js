...
import { Button, UsageLimitIndicator, PlanBadge } from '../components/shared';
...
// Search icon SVG
const SearchIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 text-gray-400">
    <circle cx="11" cy="11" r="8"></circle>
    <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
  </svg>);
// Sliders icon SVG
const SlidersIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 text-gray-400">
...
const StudyGroupsPage = () => {
    ...
    const openGroupModal = useCallback((e) => {
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
    ...
    const handleJoinGroup = async (groupPublicId) => {
        try {
            await studyGroupService.joinGroup(groupPublicId);
            // Optimistically update the UI
            setStudyGroups(prevGroups => prevGroups.map(group => group.public_id === groupPublicId
    ...
    return (<>
      ...
      <StudyGroupFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleCreateGroup} topics={topics} initialData={undefined}/>
      ...
      <div className="container mx-auto px-4 py-8">
         <motion.div variants={containerVariants} initial="hidden" animate="visible">
          <motion.div variants={itemVariants} className="mb-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
              ...
              <div className="relative" style={{ zIndex: 20 }}>
                <Button variant="primary" onClick={openGroupModal} className="mt-4 md:mt-0 relative" style={{ zIndex: 20 }}>
            ...
            <UsageLimitIndicator feature="study_groups" currentUsage={studyGroups.filter(group => group.is_member).length} showDetails={true}/>
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4">
              <div className="relative flex-grow">
                ...
                <input type="text" placeholder="Search by name, description, or topic..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"/>
                     </div>
              ...
              <div className="relative sm:w-48">
                ...
                <select value={selectedFilter} onChange={(e) => setSelectedFilter(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent appearance-none">
                  <option value="all">All Groups</option>
                  <option value="my-groups">My Groups</option>
                  <option value="public">Public</option>
                  {hasFeatureAccess('private_groups') && (<option value="private">Private Groups</option>)}
                  <option value="beginner">Beginner</option>
                  <option value="intermediate">Intermediate</option>
                  {hasFeatureAccess('advanced_analytics') && (<option value="advanced">Advanced</option>)}
                </select>
          ...
          {!isLoading && error && !isModalOpen && (<motion.div variants={itemVariants} className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 text-center">
          ...
          {/* Empty state */}
          {!isLoading && !error && filteredGroups.length === 0 && (<motion.div variants={itemVariants} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 text-center">
              ...
              <div className="relative">
                <Button variant="primary" onClick={openGroupModal} className="mx-auto">
                  <span className="flex items-center justify-center">
                    <Plus className="w-4 h-4 mr-2"/>
                    Create Group
                  </span>
          ...
          {/* Study group cards */}
          {!isLoading && !error && filteredGroups.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredGroups.map(group => (<motion.div key={group.id} variants={itemVariants}>
                  <StudyGroupCard group={group} onJoin={() => {
                    if (group.public_id) {

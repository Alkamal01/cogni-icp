...
import { BookOpen, Users, Clock, Award, TrendingUp, Plus, Sparkles, GraduationCap, Star, ArrowRight, Home } from 'lucide-react';
...
const StatCard = ({ label, value, icon: Icon, trend, color, bgGradient }) => {
    return (<motion.div variants={itemVariants} className="relative overflow-hidden rounded-lg bg-white dark:bg-gray-800 p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{label}</p>
          <p className={`text-2xl font-bold ${color} dark:text-white`}>{value}</p>
          {trend !== undefined && (<div className={`flex items-center mt-1 text-sm ${trend > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
...
const Dashboard = () => {
    ...
    const navigateToTutorSession = (tutorPublicId) => {
        if (isNavigating)
            return;
    ...
    return (<div className="container mx-auto px-4 py-8">
      <TutorFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleCreateTutor}/>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        ...
        <div className="relative z-0">
          <Button variant="gradient" onClick={openTutorModal} className="mt-4 md:mt-0 relative z-10">
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
        ...
        <motion.div variants={itemVariants} className="bg-gradient-to-r from-primary-500/10 to-purple-500/10 dark:from-primary-900/30 dark:to-purple-900/30 p-8 rounded-xl relative overflow-hidden">
      ...
      {/* Statistics Grid */}
        ...
        Array(4).fill(0).map((_, index) => (<motion.div key={index} variants={itemVariants}>
                <motion.div className="p-6 dark:bg-blue-950 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300">
        ...
        // Real stats
        statsData.map((stat, index) => (<motion.div key={index} variants={itemVariants} whileHover={{ y: -5, transition: { duration: 0.2 } }}>
                <motion.div className="p-6 dark:bg-blue-950 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300">
      ...
      {/* Main Content Grid */}
          ...
          <motion.div variants={itemVariants} className="lg:col-span-2 space-y-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-3">
                <button onClick={() => navigate('/tutors')} className="text-primary-600 dark:text-primary-400 text-sm font-medium flex items-center hover:underline">
                  See All Tutors
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {isLoading ? (<div className="col-span-2 py-16 flex justify-center">
                  <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-primary-600 dark:border-primary-400"></div>
                </div>) : tutors.length === 0 ? (<div className="col-span-2 py-16 text-center">
        ...
        tutors
            ...
            .slice(0, 4)
            .map((tutor) => (<motion.div key={tutor.public_id} whileHover={{ y: -5, transition: { duration: 0.2 } }}>
                    <Card className={`p-6 hover:border-primary-500 transition-all duration-300 dark:bg-blue-950/70 shadow-lg hover:shadow-xl group ${tutor.is_pinned ? 'border-l-4 border-l-yellow-400 dark:border-l-yellow-500' : ''}`}>
                      <div className="flex items-start space-x-4">
                        <img src={tutor.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(tutor.name)}&background=random&color=fff&size=128`} alt={tutor.name} className="w-14 h-14 rounded-full object-cover border-2 border-primary-500/30 shadow-md" onError={(e) => {
                ...
                target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(tutor.name)}&background=random&color=fff&size=128`;
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start">
                            ...
                            {tutor.is_pinned && (<Star className="w-4 h-4 text-yellow-500 fill-current" title="Pinned Tutor"/>)}
                          </div>
                          ...
                          <div className="p-4 border-t border-gray-100 dark:border-gray-700">
                            <Button variant="primary" size="sm" onClick={() => navigateToTutorSession(tutor.public_id)} className="w-full">
                              <span className="flex items-center justify-center">
                              Start Learning
          ...
          <motion.div variants={itemVariants} className="space-y-6">
            ...
            <motion.div className="p-6 dark:bg-blue-950/70 shadow-lg border border-gray-200 dark:border-gray-700">
              {isDashboardLoading ? (
        // Loading skeleton for activities
        <div className="space-y-6 animate-pulse">
                  {Array(3).fill(0).map((_, index) => (<div key={index} className="flex items-start space-x-4">
                ...
                </div>) : activities.length > 0 ? (<div className="space-y-5">
                  {activities.map((activity, index) => (<motion.div key={activity.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="flex items-start space-x-4">
              ...
              <div className="mt-6 pt-4 border-t border-gray-200 dark:border-blue-800/50">
                <Button variant="outline" className="w-full text-primary-600 dark:text-primary-400 border-primary-600/30 dark:border-primary-400/30 hover:bg-primary-50 dark:hover:bg-primary-900/20">
                  <span className="flex items-center justify-center">
                    See All Activities
                  </span>
                </Button>

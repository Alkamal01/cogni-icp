...
import { Award, MessageSquare, Users, Clock, BookOpen, Brain, Check, X } from 'lucide-react';
        ...
        <div className="md:col-span-1 space-y-6">
          <Card className="p-6">
            <div className="flex flex-col items-center text-center mb-6">
              <div className="h-32 w-32 rounded-full bg-gradient-to-r from-primary-500 to-purple-500 flex items-center justify-center text-white text-4xl font-medium shadow-lg mb-4">
                {userProfile?.avatar ? (<img src={userProfile.avatar} alt={`${userProfile.first_name} ${userProfile.last_name}`} className="h-32 w-32 rounded-full object-cover"/>) : (userProfile?.first_name?.charAt(0))}
              </div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {userProfile ? `${userProfile.first_name} ${userProfile.last_name}` : ''}
              </h1>
              <div className="flex flex-wrap justify-center gap-2 mt-3">
                {userProfile?.skills.slice(0, 3).map((skill, index) => (<span key={index} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-300">
                    {skill}
                  </span>))}
                {userProfile?.skills.length > 3 && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300">
            ...
            <div className="space-y-3 mb-6">
              {userProfile.bio && (<p className="text-gray-700 dark:text-gray-300">
                  {userProfile.bio}
                </p>)}
            ...
            <div className="mt-6 space-y-3">
              {userProfile.connectionStatus === 'none' && (<div>
                  <Button className="w-full" onClick={() => setShowMessageInput(true)} disabled={showMessageInput}>
                    <Users className="w-4 h-4 mr-2"/>
                    Connect
                  </Button>
                  
                  {showMessageInput && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-3">
                      <textarea className="w-full p-2 border border-gray-300 dark:border-gray-700 rounded-md text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 text-sm" placeholder="Add a message (optional)" rows={3} value={connectionMessage} onChange={(e) => setConnectionMessage(e.target.value)}/>
                      <div className="flex gap-2 mt-2">
                        <Button variant="primary" size="sm" className="flex-1" onClick={handleSendConnectionRequest}>
              ...
              {userProfile.connectionStatus === 'pending' && (<Button variant="outline" className="w-full" onClick={handleCancelRequest}>
                  <X className="w-4 h-4 mr-2"/>
                  Cancel Request
                </Button>)}
              
              {userProfile.connectionStatus === 'connected' && (<Button className="w-full" onClick={handleSendMessage}>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              {userProfile.badges.map((badge) => (<div key={badge.id} className="flex items-start space-x-3">
              ...
              {userProfile.badges.length === 0 && (<p className="text-gray-500 dark:text-gray-400 text-center py-4">
                  No achievements yet
                </p>)}
            </div>
          </Card>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {userProfile.skills.map((skill, index) => (<span key={index} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-300">
                  {skill}
                </span>))}
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {userProfile.interests.map((interest, index) => (<span key={index} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
                  {interest}
                </span>))}
            </div>
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-700 dark:text-gray-300 mb-6 pl-2">
              {userProfile.goals.map((goal, index) => (<li key={index}>{goal}</li>))}
            </ul>
          </Card>
          ...
          <Card className="p-6">
            ...
            <div className="mb-6">
              ...
              <div className="pl-7 space-y-2">
                {userProfile.learning.completed.map((item, index) => (<div key={index} className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-gray-100 dark:border-gray-800">
                    <span className="text-gray-900 dark:text-gray-100">{item}</span>
                  </div>))}
                
                {userProfile.learning.completed.length === 0 && (<p className="text-gray-500 dark:text-gray-400">
            ...
            <div>
              ...
              <div className="pl-7 space-y-2">
                {userProfile.learning.current.map((item, index) => (<div key={index} className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-100 dark:border-blue-900/50">
                    <span className="text-gray-900 dark:text-gray-100">{item}</span>
                  </div>))}
                
                {userProfile.learning.current.length === 0 && (<p className="text-gray-500 dark:text-gray-400">

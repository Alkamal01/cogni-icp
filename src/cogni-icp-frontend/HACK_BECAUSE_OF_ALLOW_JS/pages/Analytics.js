...
import { BookOpen, Users, Clock, Award, Brain } from 'lucide-react';
    ...
    return (<div className="p-4 sm:p-6 lg:p-8 space-y-8">
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            ...
            color: '#333'
                ...
                <Legend />
                <Line type="monotone" dataKey="minutes" stroke="#8884d8" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }}/>
              </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      <Card className="p-6">
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={analyticsData.studyTime.bySubject} dataKey="minutes" nameKey="subject" cx="50%" cy="50%" outerRadius={100} fill="#8884d8" labelLine={false} label={renderCustomizedLabel}>
                  {analyticsData.studyTime.bySubject.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                </Pie>
              <Tooltip contentStyle={{
            backgroundColor: 'rgba(255, 255, 255, 0.8)',
            backdropFilter: 'blur(5px)',
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="p-6">
            ...
            <ul className="space-y-4">
                {analyticsData.achievements.recent.map((ach, index) => (<li key={index} className="flex items-center space-x-3">
        ...
        <Card className="p-6">
            ...
            {analyticsData.learning.progressBySubject.map((sub, index) => (<div key={index}>

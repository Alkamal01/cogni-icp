...
class ZkLoginService {
   ...
   */
  generateGoogleOAuthUrl(clientId: string, redirectUrl: string, nonce: string) {
   ...
   */
  decodeJwt(encodedJwt: string): JwtPayload {
    return jwtDecode(encodedJwt) as JwtPayload;
  }
   ...
   */
  generateZkLoginAddress(jwt: string, userSalt: bigint): string {
   ...
   */
  async getZkProof(request: ZkLoginRequest): Promise<ZkLoginProofResponse> {
    try {
  ...
  async authenticateWithZkLogin(
    ...
    try {
      ...
      if (privateKeyArray.some((byte: number) => byte === null || byte === undefined || isNaN(byte))) {
   ...
   */
  async handleOAuthCallback(jwt: string) {
    try {

...
import StudyGroupForm, { StudyGroupFormData } from '../components/groups/StudyGroupForm';
...
const StudyGroups: React.FC = () => {
  ...
  const filteredGroups = studyGroups.filter(group => {
    ...
    if (selectedFilter === 'popular') return group.isPopular;
    if (selectedFilter === 'public') return group.privacy === 'public';
    if (selectedFilter === 'private') return group.privacy === 'private';
  ...
  const handleCreateGroup = (data: StudyGroupFormData) => {
  ...
  const handleJoinPrivateGroup = (accessKey: string) => {
      ...
      >
        {filteredGroups.length > 0 ? (
          filteredGroups.map((group) => (
            ...
            >
          <Card 
                className="p-6 h-full flex flex-col hover:shadow-xl transition-all duration-300 border-0 shadow-lg dark:bg-blue-950/70 overflow-hidden"
          >
            ...
            <div className="flex items-start justify-between">
                ...
                {group.name}
                      {group.privacy === 'private' && (
                        <Lock className="h-4 w-4 ml-2 text-gray-500 dark:text-gray-400" />
                      )}
              </h3>
              <span 
                      className={`px-2.5 py-1 rounded-full text-xs font-medium ${skillLevelColors[group.skill_level]}`}
              >
                      {group.skill_level.charAt(0).toUpperCase() + group.skill_level.slice(1)}
              </span>
            </div>
                  {group.isPopular && (
                    <span className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 bg-yellow-500 text-white text-xs font-bold px-2 py-1 rounded-full flex items-center">
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center">
                  <div className="flex items-center space-x-2">
                    <div className="flex">
                      {group.members.slice(0, 3).map((member, i) => (
                        <div 
                          key={i}
                          className={`w-8 h-8 rounded-full bg-gradient-to-br from-primary-${(i+3)*100} to-purple-${(i+3)*100} flex items-center justify-center text-white text-xs font-bold border-2 border-white dark:border-gray-800`}
                      ...
                      {group.members.length > 3 && (
                        <div 
                          className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-700 dark:text-gray-300 text-xs font-bold border-2 border-white dark:border-gray-800"
                  ...
                  <Button
                    size="sm"
                    variant={group.members.includes("You") ? "primary" : (group.privacy === "private" ? "secondary" : "outline")}
                    onClick={() => handleJoinGroup(group)}
                    disabled={group.members.includes("You") && group.members.length >= group.max_members}
                    icon={group.privacy === "private" && !group.members.includes("You") ? <Lock className="w-4 h-4 mr-1" /> : undefined}
                  >

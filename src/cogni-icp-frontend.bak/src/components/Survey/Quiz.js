...
const Quiz = ({ topicId }) => {
  ...
  return (
    <div className="p-8 bg-[#fff] text-[#000053] w-full h-full rounded-lg shadow-lg">
          ...
          {submitted ? (
            ...
            <div>
              ...
              {quizData.map((question, index) => (
                <div key={index} className="mb-6 p-4 bg-[#1a1a5e] rounded-lg shadow-md">
                  <p className="text-lg font-medium">{question.question}</p>
                  <div className="mt-2">
                    {question.options.map((option, optionIndex) => (
                      <label
                        key={optionIndex}
                        className="block mt-1 p-2 bg-[#2a2a6e] rounded cursor-pointer hover:bg-gray-700 transition"
                      >
                        <input
                          type="radio"
                          name={`question-${index}`}
              ...
              <button
                ...
                className="bg-gray-500 hover:bg-gray-400 text-white py-2 px-6 rounded-lg shadow-lg transition mt-4"
              >
                Submit Answers
              </button>
            </div>
